...
def plot_array_comparison(
    array1, array2, array_names=["Array 1", "Array 2"], bus_names=None, title_prefix="Array Comparison"
):
    """
...
def detailed_comparison_stats(array1, array2, array_names=["Array 1", "Array 2"]):
    ...
    for phase in range(3):
        ...
        # Basic statistics
        print(f"{array_names[0]:15}", f"Mean: {np.mean(arr1[:, phase]):8.4f}", f"Std: {np.std(arr1[:, phase]):8.4f}")
        print(f"{array_names[1]:15}", f"Mean: {np.mean(arr2[:, phase]):8.4f}", f"Std: {np.std(arr2[:, phase]):8.4f}")

        # Differences
        diff = arr1[:, phase] - arr2[:, phase]
        abs_diff = np.abs(diff)

        print(f"{'Difference':15}", f"Mean: {np.mean(diff):8.4f}", f"Std: {np.std(diff):8.4f}")
        print(f"{'Abs Difference':15}", f"Mean: {np.mean(abs_diff):8.4f}", f"Max: {np.max(abs_diff):8.4f}")

        # Correlation
        correlation = np.corrcoef(arr1[:, phase], arr2[:, phase])[0, 1]
        print(f"{'Correlation':15}", f"r = {correlation:.6f}")

        # Percentage differences
        percent_diff = np.abs(diff) / np.maximum(np.abs(arr1[:, phase]), 1e-10) * 100
        print(f"{'% Difference':15}", f"Mean: {np.mean(percent_diff):8.2f}%", f"Max: {np.max(percent_diff):8.2f}%")
        ...
        if np.any(mask_show):
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    y=pred_shown,
                    mode="lines+markers",
                    marker=dict(symbol="circle", size=8, color=phase_colors[phase], line=dict(color="white", width=2)),
            ...
            fig.add_trace(
                go.Scatter(
                    ...
                    mode="lines+markers",
                    marker=dict(
                        symbol="triangle-up", size=10, color="white", line=dict(color=phase_colors[phase], width=3)
                    ),
            ...
            bar_colors = ["red" if abs(d) > np.std(differences) else "lightgray" for d in differences]
            ...
            fig.add_trace(
                go.Bar(
                    ...
                    marker=dict(color=bar_colors, opacity=0.6, line=dict(color="rgba(0,0,0,0.3)", width=1)),
            ...
            fig.update_yaxes(
                range=[0, max_valInjection + 3], title_text="DER Injection (kW)", row=row, col=1, secondary_y=False
            )
...
def create_detailed_plots(output, pred, measurement, array_names=["Predicted", "Measured"]):
    ...
    for phase in range(ph):
        ...
        if np.any(mask_show):
            ...
            np.sqrt(np.mean((pred_shown - measurement_shown) ** 2))
            np.mean(np.abs(pred_shown - measurement_shown))
            # rmse = np.sqrt(np.mean((pred_all - measurement_all) ** 2))
            # mae = np.mean(np.abs(pred_all - measurement_all))
            ...
            ax.text(
                0.5,
                0.5,
                f"No prediction values ≥ 0.2 in {phase_names[phase]}",
        ...
        ax.set_title(f"{phase_names[phase]} - Filtered Comparison", fontsize=14, fontweight="bold")
        ax.set_xlabel("Bus Index")
        ...
        if np.any(mask_show):
            ...
            # Color bars by difference magnitude
            colors = ["red" if abs(d) > np.std(differences) else "lightgray" for d in differences]

            # Get the maximum difference for reference
            max_diff = np.max(np.abs(differences))
            print("max_diff", max_diff)
            # Create bars that start from top and go down
            ax2.bar(x_shown, differences, alpha=0.4, color=colors, width=0.8, bottom=max_diff)  # Start bars from top
    ...
    plt.tight_layout()
    plt.savefig(f"{output}/Imputed_data_Detection.png", dpi=300, bbox_inches="tight")
    # plt.show()
...
def create_detailed_plots_allNodes(pred, measurement, array_names=["Predicted", "Measured"]):
    ...
    for phase in range(3):
        ...
        ax.fill_between(x_indices, arr1[:, phase], arr2[:, phase], alpha=0.3, color=phase_colors[phase])
...
if __name__ == "__main__":
    ...
    fig2 = create_detailed_plots(output, predicted_injections, injection_data, array_names=["Predicted", "Actual"])
